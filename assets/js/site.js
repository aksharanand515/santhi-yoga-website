...
(function () {
  ...
  var lastY = window.scrollY || 0, travelled = 0;
  function onScroll() {
    var y = window.scrollY || window.pageYOffset;
    var max = Math.max(1, root.scrollHeight - window.innerHeight);
    // Scroll progress drives the gold thread and the sun's slow descent
    root.style.setProperty('--sp', Math.min(1, Math.max(0, y / max)).toFixed(4));
    if (header) {
      header.classList.toggle('is-scrolled', y > 10);
      // Step aside while reading down the page; return on the first scroll up
      var dy = y - lastY;
      travelled = (dy > 0) === (travelled > 0) ? travelled + dy : dy;
      var hide = y > window.innerHeight * 0.6 && travelled > 60 && !document.body.classList.contains('nav-open');
      if (travelled < -30 || y < window.innerHeight * 0.6) hide = false;
      else if (!hide) hide = header.classList.contains('is-hidden');
  ...
  /* ---------- Page to page ----------
     Browsers with cross-document view transitions animate between pages in
     CSS alone. Elsewhere a veil of warm light fills the screen before the
     next page loads, and lifts again when it arrives (or returns from the
     back-forward cache). */
  (function () {
    if (reduceMotion || 'onpagereveal' in window) return;
    ...
    document.addEventListener('click', function (e) {
      ...
      if (url.pathname === location.pathname && url.search === location.search) return; // same page, or a hash on it
      e.preventDefault();
  ...
  /* ---------- Lamp light ----------
     A soft pool of warm light follows a fine pointer across the page, the
     way a brass lamp carried through the pavilion would. */
  (function () {
    if (reduceMotion || !finePointer) return;
    var lamp = document.createElement('div');
    lamp.className = 'lamp'; lamp.setAttribute('aria-hidden', 'true');
    document.body.appendChild(lamp);
    var x = -999, y = -999, tx = x, ty = y, raf = 0;
    function tick() {
  ...
  // Buttons: a magnetic pull towards the pointer, the label trailing a little further
  function initMagnetic() {
    if (!finePointer) return;
    $$('[data-magnetic], .btn:not(.btn--sm), .icon-btn, .float-wa, .hero-ttc').forEach(function (btn) {
      if (btn.closest('.pop')) return;
  ...
  // Inner page hero: the clip reveal plays in CSS; the photo drifts as the hero
  // scrolls away, the words lift a little faster, and the sun sinks behind them
  function initPageHero() {
    var ph = $('.phero');
    if (!ph) return;
    var st = { trigger: ph, start: 'top top', end: 'bottom top', scrub: true };
    var img = $('.phero-media img');
    if (img) gsap.fromTo(img, { '--py': '0%' }, { '--py': '8%', ease: 'none', scrollTrigger: st });
    var media = $('.phero-media');
    if (media) gsap.to(media, { y: 60, ease: 'none', scrollTrigger: st });
    var words = $('.phero .grid-2 > div:first-child') || $('.phero .container > div');
    if (words) gsap.to(words, { y: -50, autoAlpha: 0.2, ease: 'none', scrollTrigger: { trigger: ph, start: 'top top', end: 'bottom 20%', scrub: true } });
  }
  ...
  function initParallax() {
    ...
    $$('[data-speed]').forEach(function (el) {
      ...
      gsap.fromTo(el, { y: speed * 120 }, { y: speed * -120, ease: 'none', scrollTrigger: { trigger: el.parentNode, start: 'top bottom', end: 'bottom top', scrub: true } });
    });
  }

  // The practices on the horizon: a steady drift that quickens and leans with the scroll
  ...
  // Words that fill with light as they are read: each word warms from haze to ink
  function initLightText() {
    $$('[data-light]').forEach(function (el) {
      var words = [];
      (function walk(node) {
        Array.prototype.slice.call(node.childNodes).forEach(function (n) {
          if (n.nodeType === 3) {
            ...
            n.textContent.split(/(\s+)/).forEach(function (w) {
              ...
              var s = document.createElement('span'); s.className = 'lw'; s.textContent = w; words.push(s); frag.appendChild(s);
            });
            n.parentNode.replaceChild(frag, n);
          } else if (n.nodeType === 1) walk(n);
        });
      })(el);
      gsap.fromTo(words, { opacity: 0.16 }, { opacity: 1, ease: 'none', stagger: 0.1, scrollTrigger: { trigger: el, start: 'top 82%', end: 'bottom 45%', scrub: 0.6 } });
    });
  }

  // The practice hall: a window onto the hall opens to fill the screen as it
  // is scrolled into, then the words arrive in the light
  function initHall() {
    var hall = $('.hall');
    if (!hall) return;
    var win = $('.hall-window', hall), img = $('.hall-window img', hall), copy = $('.hall-copy', hall);
    var mm = gsap.matchMedia();
    mm.add({ wide: '(min-width: 800px)', narrow: '(max-width: 799px)' }, function (ctx) {
      var wide = ctx.conditions.wide;
      var tl = gsap.timeline({ scrollTrigger: wide
        ? { trigger: hall, start: 'top top', end: '+=110%', pin: true, scrub: 0.8, anticipatePin: 1 }
        : { trigger: hall, start: 'top 85%', end: 'top 10%', scrub: 0.6 } });
      tl.fromTo(win, { '--ci': wide ? '14% 22%' : '8% 6%', '--cr': wide ? '360px' : '200px' }, { '--ci': '0% 0%', '--cr': '0px', ease: 'power2.inOut', duration: 1 }, 0)
        .fromTo(img, { scale: 1.28 }, { scale: 1, ease: 'power2.out', duration: 1 }, 0)
        .fromTo(copy, { autoAlpha: 0, y: 60 }, { autoAlpha: 1, y: 0, ease: 'power3.out', duration: 0.5 }, wide ? 0.55 : 0.3);
      if (wide) tl.to({}, { duration: 0.35 });
      return function () { gsap.set([win, img, copy], { clearProps: 'all' }); };
    });
  }

  // The closing window onto the pavilion opens as it rises into view
  function initCtaWindow() {
    $$('.cta-window').forEach(function (w) {
      gsap.fromTo(w, { '--open': 0 }, { '--open': 1, ease: 'none', scrollTrigger: { trigger: w, start: 'top bottom', end: 'top 20%', scrub: 0.6 } });
      var bg = $('.cta-bg', w);
      if (bg) gsap.fromTo(bg, { yPercent: -8 }, { yPercent: 8, ease: 'none', scrollTrigger: { trigger: w, start: 'top bottom', end: 'bottom top', scrub: true } });
    });
  }

  // Cards and photographs lean towards the pointer, with light moving across them
  function initTilt() {
    if (!finePointer) return;
    $$('[data-tilt], .card, .post-card .frame, .teacher-portrait .frame, .phero-media .frame, .journey-card .frame, .tile-btn').forEach(function (el) {
      el.setAttribute('data-tilt', '');
      var max = el.classList.contains('card') ? 5 : 7;
      ...
      el.addEventListener('pointerleave', function () { el.classList.remove('is-tilting'); el.style.setProperty('--rx', '0deg'); el.style.setProperty('--ry', '0deg'); });
    });
  }
  ...
  $$('.btn').forEach(function (btn) {
    if ((btn.classList.contains('btn--primary') || btn.classList.contains('btn--gold')) && !btn.classList.contains('btn--sm') && !reduceMotion) {
      var orbit = document.createElement('span'); orbit.className = 'btn-orbit'; orbit.setAttribute('aria-hidden', 'true');
    ...
    btn.addEventListener('pointerdown', function (e) {
      ...
      dot.className = 'btn-ripple'; dot.setAttribute('aria-hidden', 'true');
